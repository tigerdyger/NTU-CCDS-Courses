...
\begin{figure}[htbp]
  \centering
  \resizebox{0.94\textwidth}{!}{\input{figures/text-processing-levels}}
...
\begin{figure}[htbp]
  \centering
  \resizebox{0.92\textwidth}{!}{\input{figures/regex-structure}}
...
\subsubsection{Shorthand Character Classes}

常见 shorthand classes（简写字符类）包括 \texttt{\detokenize{\d}}/\texttt{\detokenize{\D}}（数字/非数字）、\texttt{\detokenize{\w}}/\texttt{\detokenize{\W}}（单词/非单词字符）和 \texttt{\detokenize{\s}}/\texttt{\detokenize{\S}}（空白/非空白字符）。其 ASCII/Unicode 范围依 regex dialect（正则方言）与运行模式而异；\texttt{\detokenize{{n,}}} 表示前一项至少重复 $n$ 次。
