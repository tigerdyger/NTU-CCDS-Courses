...
OpenAI Codex was used for debugging, figure generation and report polishing.
...
\]
The selected A1 architecture and training hyperparameters were fixed. Seed 42
and the 2017--2020/2021 split were used only to select $\lambda$ and A3's epoch
budget; the lowest validation RMSE, 58,076.59 SGD, occurred at $\lambda=2$ and
epoch 3. The five final runs therefore retrained on 2017--2021 for three epochs.
...
\midrule
Baseline ReLU & 98,059.90 & 3,498.80 & 0.6680 & 0.0240 \\
Wide-and-deep ($\lambda=2$) & 102,507.98 & 4,121.13 & 0.6370 & 0.0292 \\
\bottomrule
\end{tabular}
\end{center}
...
robust test benefit: across five seeds its mean RMSE was 4,448.08 SGD (4.54\%)
...
Both models had the same top-three feature set: \texttt{floor\_area\_sqm},
\texttt{flat\_model\_type}, and \texttt{remaining\_lease\_years}. Their mean
absolute attributions in that order were 67,840, 45,104, and 42,465 SGD for the
baseline, and 73,417, 37,776, and 47,252 SGD for wide-and-deep; the latter
ranked remaining lease above flat model/type. This agreement on the set
suggests that the added linear path did not fundamentally change the dominant
predictive signals, although it redistributed their magnitudes and some
attribution among location features.
