...
- 为解释课程知识，可以使用讲义、课件或参考答案中确有助于理解的必要图示；应只截取相关部分，并注明课程、原始文件及页码或 slide，避免整页堆叠或大段转载。公开发布前应确认使用范围适合教学、评论或已获得相应许可。
- 不得加入整份课程课件、录像、转写稿、学习平台导出、正式考试、成绩、学号、凭据或尚未完成并核对的受考核答案。
- 原始材料应保存在已被忽略的本地材料目录中。
- 图片可以是原创图，也可以是带清楚出处的必要原图裁切；优先选择科学正确、清晰且能直接帮助理解的版本。
